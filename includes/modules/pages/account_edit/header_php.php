...
/*-bof-c-zcbb*/
    if ($nick != '' && is_object($bb) && $bb->check_email_not_used(zen_db_input($email_address), $nick) != bb::STATUS_OK) {
      $error = true;
      if (method_exists($bb, 'get_bb_message'))
      $messageStack->add('account_edit', (method_exists($bb, 'get_bb_message') && $bb->get_bb_message() != '') ? $bb->get_bb_message() : (ENTRY_EMAIL_ADDRESS_ERROR_EXISTS . ' (' . $bb->get_bb_name() . ')') );
/*-eof-c-zcbb*/
    }
/*-bof-d-zcbb
  }
  ...
  if ($error == false) {
/*-bof-c-zcbb*/
    // update bulletin board with new address
    if (is_object($bb) && $nick != '') {   
      $bb->change_email(zen_db_input(zen_db_input($nick)), zen_db_input($email_address));
    }
/*-eof-c-zcbb*/
